...
class FtpDownloader(object):
    ...
    def download_folder(self, folder='', target_folder=''):
        ...
        files, folders = self.listcontents(folder)
        for fl in files:
            self.download(join(folder, fl), target_folder)
        for fld in folders:
            destination = join(target_folder, fld)
            self.download_folder(join(folder, fld), destination)

    def download(self, fl, target_folder=''):
        ''' Downloads a file from the ftp server to target folder

        @param fl: the absolute path for the file on the server
        ...
        folder = '/'.join(fl.split('/')[:-1])
        self.move_to_folder(folder)
        destination = join(target_folder, fl)
        fl = fl.split('/')[-1]
        create_folders('/'.join(destination.split('/')[:-1]))
        if not exists(destination):
            try:
                with open(destination, 'wb') as result:
                    self._ftp.retrbinary('RETR %s' % (fl,), result.write)
            except error_perm as e:  # fl is a folder
                print e
                remove(join(target_folder, fl))
    ...
    def move_to_folder(self, folder):
        ''' Changes the working directory on the server.
            ...
            @type folder: string
            '''
        for subfloder in folder.split('/'):
            if subfloder:
                self._ftp.cwd(subfloder)

    def listcontents(self, folder=''):
        ''' Lists the files and folders of a specific directory
            ...
            @returns: a tuple with the list of files in the folders
